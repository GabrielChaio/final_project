...
10. [已知限制與預留功能](#10-已知限制與預留功能)
    - [10.1 現有限制與已實作功能概覽](#101-現有限制與已實作功能概覽)
    - [10.2 未來期望功能（待實作）](#102-未來期望功能待實作)
...
| 探測次數 | 0 ≤ 值 ≤ 99 | 「探測次數不能為負數」／「探測次數不能超過99」 |
...
| 欄位 | 限制 | 實作方式 |
|------|------|----------|
| 玩家 ID | 最多 10 字元 | `tk.Entry` 綁定 `validate="key"` + `_validate_id_length('%P')` |

`_validate_id_length(predicted_text) → bool`：當預測文字長度 ≤ 10 時回傳 `True`（允許輸入），否則回傳 `False`（封鎖輸入）。
...
遊戲以「失敗（踩到地雷）」結束時，顯示「反悔」按鈕，讓玩家撤銷最後一步並繼續遊戲。

**行為規格：**

1. 「反悔」按鈕僅在遊戲失敗後的結束對話框中出現。
2. 反悔無次數限制，可連續反悔多步。
3. **第一次反悔時**彈出一次性提醒視窗：
   > 「有反悔過的遊戲紀錄一樣能儲存，但是不能上傳至排行榜。」
4. 有反悔過的遊戲，結束後仍可「儲存遊戲紀錄」，但「上傳至排行榜」按鈕強制停用。
5. 此類遊戲紀錄在「回放記錄」清單的「結果」欄位顯示 **「反悔」**（不顯示「勝利」或「失敗」）。
...
- 新增 `undo_used: bool` 旗標於 `MinesweeperUI`，首次反悔時設為 `True`。
- 反悔時需還原：`board`、`revealed`、`history`、`radar_mines`、`flag_count`、`radar_uses_left` 至上一步操作前的狀態。最簡單的方式為重新 seek 至 `history[-2]`（即回到倒數第二步）。
- 回放 meta 中 `result` 欄位寫入 `"undo"` 而非 `"win"` 或 `"lose"`。
- `_RESULT_LABEL` 新增 `"undo": "反悔"` 對應。

---

#### 遊戲紀錄本機下載與匯入

讓玩家可將雲端回放以 JSON 格式下載至本機，或從本機 JSON 檔案載入回放，無需網路連線。

**行為規格：**

1. **下載**：在「回放記錄」視窗的操作按鈕列新增「下載」按鈕，點擊後以 `tkinter.filedialog.asksaveasfilename` 選擇儲存路徑，將選取的回放 JSON 寫入本機。
2. **匯入**：在主選單新增「本機回放」按鈕（或於「回放記錄」視窗新增「從本機載入」按鈕），以 `tkinter.filedialog.askopenfilename` 選取 JSON 檔案，驗證格式後直接進入回放模式，不需登入。

**實作要點：**

- 下載時直接將 `full_data` dict 序列化為 JSON（`json.dump`），無需額外轉換。
- 匯入時驗證必要欄位（`version`、`board`、`history`）後呼叫現有 `play_replay(data)`。
- 不需新增 Firebase 路徑，完全本機操作。

---

#### 遊戲紀錄分享與查詢

讓玩家可透過「回放 ID」將特定遊戲紀錄分享給其他玩家查看，無需對方登入。

**行為規格：**

1. **分享**：在「回放記錄」視窗的操作按鈕列新增「分享」按鈕，點擊後以對話框顯示該筆回放的 Firebase record ID（格式如 `-OQXxxxxxxx`），並提供「複製」按鈕。
2. **查詢**：在主選單或「回放記錄」視窗新增「查詢回放」功能，玩家輸入他人分享的「玩家 ID + 回放 ID」（或統一格式的分享碼），系統呼叫 `firebase_client.get_replay_by_id(player_id, record_id)` 取得回放資料並播放，**不需登入**。
...
- 查詢時呼叫已存在的 `fb.get_replay_by_id(player_id, record_id)` 函式。
- 驗證格式與必要欄位後呼叫現有 `play_replay(data)`。
- 分享碼可直接貼入輸入框，以 `/` 分割解析 player_id 與 record_id。
- 查詢功能不需登入（Firebase `records/$pid` 的 `.read: true` 規則已允許匿名讀取）。
